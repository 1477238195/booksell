        ...
        if (StringUtils.isNotBlank(contentType) && contentType.contains("multipart/form-data")) {
            if (checkInWhiteList(appProperties.getWhiteList(), uri)) {
                if (isTokenValid(token)) {
                    HeaderRequestWrapper w = new HeaderRequestWrapper(httpServletRequest);
                    writeUserInfo(w, token);
            ...
            HeaderRequestWrapper multipartWrapper = new HeaderRequestWrapper(httpServletRequest);
            if (!writeUserInfo(multipartWrapper, token)) {
                ResponseUtil.write(response, R.createByNeedLogin());
                return;
            }
            chain.doFilter(multipartWrapper, response);
            return;
        }
