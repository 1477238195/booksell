      ...
      <el-table :data="tableData" v-loading="loading" border stripe>
        ...
        <el-table-column label="操作" width="280" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === 1" link type="success" size="small" @click="handlePay(row)">支付</el-button>
            <el-button v-if="row.status === 1 || row.status === 2" link type="warning" size="small" @click="handleCancel(row)">取消订单</el-button>
            <el-button v-if="row.status === 3" link type="success" size="small" @click="handleConfirm(row)">确认收货</el-button>
          </template>
        </el-table-column>
      </el-table>
...
const handleQuery = async () => {
  loading.value = true
  try {
    const res = await OrderApi.myBuyOrders(queryForm)
    if (res.status === 0) {
...
const handlePay = async (row) => {
  try {
    ...
    const res = await OrderApi.payOrder(row.orderId)
    if (res.status === 0) {
