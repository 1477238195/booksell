      ...
      <el-table :data="tableData" v-loading="loading" border stripe>
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === 2" link type="success" size="small" @click="handleDeliver(row)">发货</el-button>
...
const handleQuery = async () => {
  loading.value = true
  try {
    const res = await OrderApi.mySellOrders(queryForm)
    if (res.status === 0) {
...
const handleDeliver = async (row) => {
  try {
    ...
    const res = await OrderApi.deliverOrder(row.orderId)
    if (res.status === 0) {
